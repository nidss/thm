    ...
    <main className="relative flex min-h-dvh flex-col items-center overflow-hidden px-4 py-12 sm:py-16">
      ...
      <header className="mb-10 flex max-w-xl flex-col items-center text-center">
        ...
        <p className="mt-3 text-pretty text-sm leading-relaxed text-white/55">
          ตัวอย่างหน้าจอแอป ThaiMove ที่ทำจากดีไซน์ใน Figma
          กดเมนูลอยด้านล่างของเครื่องเพื่อสลับดูแต่ละหน้าได้เลย
        </p>
      </header>
      ...
      <section className="mt-10 flex max-w-md flex-wrap items-center justify-center gap-2">
        {TABS.map((tab) => (
          <span
            key={tab.key}
            className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] text-white/50"
          >
            {tab.label}
          </span>
        ))}
